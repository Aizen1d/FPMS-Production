...
<div class="container-fluid margin">
    ...
    <div class="container-fluid task-list" style="position: relative;">
        ...
        <div class="row task-row">
            <div class="col-6">
                <h5 class="task-row-content my-2 task-name-text" style="text-align: left; margin-left: 46.5%">{{ $task->task_name }}</h5>
            </div>
            <div class="col-3">
                <h5 class="task-row-content my-2 date-created" style="text-align: left; margin-left: 35.5%">{{ date('F j, Y', strtotime($task->created_at)) }}<br>{{ date('g:i A', strtotime($task->created_at)) }}</h5>
            </div>
            <div class="col-3">
                @if (Carbon\Carbon::parse($task->due_date)->isPast())
                <h5 class="task-row-content my-2 text-danger due-date" style="text-align: left; margin-left: 39.5%">{{ date('F j, Y', strtotime($task->due_date)) }}<br>{{ date('g:i A', strtotime($task->due_date)) }}</h5>
                @else
                <h5 class="task-row-content my-2 due-date" style="text-align: left; margin-left: 39.5%">{{ date('F j, Y', strtotime($task->due_date)) }}<br>{{ date('g:i A', strtotime($task->due_date)) }}</h5>
                @endif
            </div>
        </div>
        @endforeach
        <div id="clickable-rows" data-clickable-rows="{{ json_encode($clickableRows) }}" style="display: none;"></div>
    ...
    <script>
        ...
        const debouncedInputHandler = debounce(function(event) {
            ...
            fetch(`/faculty-home/department/assigned-tasks/search?query=${query}`)
                .then(response => response.json())
                .then(newTasks => {
                    tasks = newTasks;
                    clickable = newTasks.clickableRows;
                    newlyAdded = null;
        ...
        const clickableRowsElement = document.querySelector('#clickable-rows');
        const clickableRows = JSON.parse(clickableRowsElement.dataset.clickableRows);
        ...
        for (let i = 0; i < getTaskRows.length; i++) {
            ...
            // Check if task name is in clickableRows
            if (clickableRows.includes(getTaskName)) {
                // Add event listener
                ...
                getTaskRow.classList.add('unclickable-row');
            }
        }
        ...
        function renderTasks() {
            ...
            for (let i = 0; i < tasks.fetchTasks.length; i++) {
                const task = tasks.fetchTasks[i];
