...
@section('styles')
<link rel="stylesheet" type="text/css" href="{{ asset('admin/css/admin_tasks_seminars_view.css') }}">
@endsection

@section('body')
<div class="overlay"></div>
@include('layouts.admin_navbar')
@include('layouts.admin_selected_seminar_sidebar') 
@include('layouts.notification_side')

<div class="container-fluid margin">
    <div class="row">
        <div class="col-6">
            <h3 class="my-4 title">Seminars</h3>
        </div>
    ...
    <script>
              ...
              (function(f) {
                  clickableElement.addEventListener('click', function() {
                    if (additionalFiles.includes(f)) {
                        ...
                        let url = `/admin-tasks/seminars/attachment/preview?id=${itemId}&fileName=${f.name}`;
      ...
      fetch(`/admin-tasks/seminars/getAttachments?id=${itemId}`)
        .then(response => response.json())
        .then(files => {
          // Get the files
        ...
        function submitForm() {
            ...
            fetch(`/admin-tasks/seminars/update`, {
                headers: {
        ...
        function deleteItem() {
            ...
            let token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            fetch(`/admin-tasks/seminars/delete`, {
                headers: {
                    "Accept": "application/json, text-plain, */*",
                    "X-Requested-With": "XMLHttpRequest",
                    "X-CSRF-TOKEN": token
            ...
            .then(data => {
                if (data.message) {
                    window.location.href = '/admin-tasks/seminars';
                } 
                else {
